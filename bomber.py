...
def check_intr():
    try:
        requests.get("https://toptrendyproducts.in")
    except Exception:
        bann_text()
...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("FUCKER was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update FUCKER.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com")
...
def check_for_updates():
    ...
    fver = requests.get(
            "https://raw.githubusercontent.com/yogeshjoga/fucker-bomb/main/.version"
            ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("FUCKER is up-to-date")
        mesgdcrt.GeneralMessage("Starting FUCKER")


def notifyen():
    try:
        noti = requests.get(
            "https://raw.githubusercontent.com/yogeshjoga/fucker-bomb/main/.notify"
            ).text.upper()
        if len(noti) > 10:
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SectionMessage("FUCKING is in progress - Please be patient")
    mesgdcrt.GeneralMessage(
        "Please stay connected to the internet during FUCKING your ENEMY")
    mesgdcrt.GeneralMessage("Target       : " + cc + " " + target)
    mesgdcrt.GeneralMessage("Sent         : " + str(requested))
    mesgdcrt.GeneralMessage("Successful   : " + str(success))
    mesgdcrt.GeneralMessage("Failed       : " + str(failed))
    mesgdcrt.WarningMessage(
        "This tool was made for fun and REVANGE purposes only")
    mesgdcrt.SuccessMessage("FUCKER was created by JOGA YOGESH")
...
def workernode(mode, cc, target, count, delay, max_threads):
            ...
            for job in as_completed(jobs):
                ...
                if result is None:
                    mesgdcrt.FailureMessage(
                        "FUCKING ENMY limit for your target has been reached")
    ...
    print("\n")
    mesgdcrt.SuccessMessage("FUCKING completed!")
    time.sleep(1.5)
    bann_text()
    sys.exit()
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit = {"sms": 500000, "call": 1000, "mail": 200000}
        cc, target = "", ""
        if mode in ["sms", "call"]:
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("FUCKING will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['FUCKING', 'BIKER', 'YOGESH', 'BILLA']
...
FUCKER can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

FUCKER is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by SpeedX !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start FUCKER with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start FUCKER with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start FUCKER with MAIL Bomb mode")
parser.add_argument("-u", "--update", action="store_true",
                    help="update FUCKER")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current FUCKER contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current FUCKER version")
